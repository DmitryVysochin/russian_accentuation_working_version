...
def tokenize(text, wordforms):
    res = []
    doc = ru_nlp(text)#токенизация spaCy
    for token in doc:#составляем требуемый нам токен из полученых и ищем подхадящую словоформу из подготовленных данных
        if token.pos_ != 'PUNCT':
...
lemmas, wordforms = load()                      # загрузка лемм и словоформ

f = open("in.txt", mode='r', encoding='utf-8')  #читаем текст из файла in.txt
sentence = f.read()
f.close()
